...
def k_to_c(T):
    ...
    temperature-unit reporting). Mirrors FEM+GravityInversion.py's k_to_c;
    duplicated here rather than imported to keep this script self-contained
    (its filename lacks the '+' that forces the other module's importlib
    workaround, but the project convention is that final_codes/ scripts do
...
def run_one_solution(label: str, grav: dict,
                     N_mc: int = 100_000, seed: int = 42,
                     n_ensemble: int = 300) -> dict | None:
    """
    Full pipeline for one published J2 solution.

    In addition to the FEM solve for the median structure, this also solves
    the FEM problem for up to `n_ensemble` accepted structures (subsampled
    if more are available) so the comparison plot can show a 16-84%
    credible envelope per gravity solution rather than a single median
    curve (R2 minor 8: "Update Figure 4 and Section 5.4 to include shaded
    16-84% credible posterior envelopes for all four historical gravity
    solutions ... instead of single median curves").
    ...
    # Key temperatures
    T_core_C   = k_to_c(T_med[nearest_idx(r_med, 0)])
    T_mantle_C = k_to_c(T_med[nearest_idx(r_med, R_mantle_med)])
    T_ocean_C  = k_to_c(T_med[nearest_idx(r_med, R_ocean_med)])
    ...
    # --- FEM ensemble (for the 16-84% credible envelope) -------------------
    n_sub = min(n_ensemble, n_acc)
    rng_sub = np.random.default_rng(seed)
    sub_idx = rng_sub.choice(n_acc, size=n_sub, replace=False)
    print(f"  Computing FEM thermal profiles for {n_sub} ensemble members "
          f"(credible envelope) ...")
    T_matrix_on_med = np.empty((n_sub, len(r_med)))
    for k, i in enumerate(sub_idx):
        r_i, T_i = compute_temperature(*solutions[i])
        T_matrix_on_med[k] = np.interp(r_med, r_i, T_i)
        if (k + 1) % max(1, n_sub // 5) == 0:
            print(f"    {k+1}/{n_sub}", end="\r", flush=True)
    print()
    ...
    return {
        ...
        "T_med"         : T_med,
        "diag"          : diag,
        "T_matrix"      : T_matrix_on_med,   # (n_sub, len(r_med)) ensemble, on r_med grid
    }
...
def plot_comparison(results: list[dict], save_path: str | None = None) -> None:
    """
    Plot all median temperature profiles on a single panel, each surrounded
    by its 16-84% credible envelope across the accepted Monte-Carlo ensemble
    (R2 minor 8) rather than showing the median curve alone.
    ...
    # ── Draw one shaded envelope + median line per accepted solution ───────
    for res in results:
        ...
        if "T_matrix" in res and res["T_matrix"] is not None and len(res["T_matrix"]) > 1:
            T_lo = k_to_c(np.percentile(res["T_matrix"], 16, axis=0))
            T_hi = k_to_c(np.percentile(res["T_matrix"], 84, axis=0))
            ax.fill_betweenx(depth, T_lo, T_hi, color=col, alpha=0.15, lw=0)
    ...
    ax.set_title("Europa Interior Temperature by Gravity Solution\n"
                 "(median line, shaded band = 16-84% credible envelope)",
...
def print_summary_table(results: list[dict]) -> None:
    ...
    for res in results:
        ...
        tc   = k_to_c(res["T_med"][nearest_idx(res["r_med"], 0)])
        num  = res["diag"]["Nu_m"]
...
def analyze_envelope_overlap(results: list[dict]) -> None:
    """
    Quantify where the per-solution 16-84% credible envelopes overlap vs.
    diverge, rather than comparing median curves alone (R2 minor 8 / R2
    section-5.4 comment: "comparisons of different gravity solutions should
    not just look at the median profile ... Are any models significantly
    different, or do most temperature profile regions overlap?").

    For each pair of solutions, interpolates both envelopes onto a common
    depth grid and reports the fraction of that grid where the bands
    overlap, plus the overlap status at three reference depths (ice base,
    seafloor, core centre -- taken from the Anderson+1998 median structure,
    consistent with the layer-boundary reference used in plot_comparison).
    """
    have_matrix = [r for r in results if "T_matrix" in r and r["T_matrix"] is not None
                   and len(r["T_matrix"]) > 1]
    if len(have_matrix) < 2:
        print("\n  [Envelope overlap analysis skipped: need >=2 solutions with ensemble T_matrix]")
        return

    depth_grid = np.linspace(0, R_EUROPA / 1e3, 300)

    def bands_on_grid(res):
        r_med = res["r_med"]
        depth = (R_EUROPA - r_med) / 1e3          # decreasing as r increases
        T_lo  = k_to_c(np.percentile(res["T_matrix"], 16, axis=0))
        T_hi  = k_to_c(np.percentile(res["T_matrix"], 84, axis=0))
        order = np.argsort(depth)                  # ascending depth for np.interp
        lo = np.interp(depth_grid, depth[order], T_lo[order])
        hi = np.interp(depth_grid, depth[order], T_hi[order])
        return lo, hi

    bands = {res["label"]: bands_on_grid(res) for res in have_matrix}

    ref = next((r for r in have_matrix
                if "Anderson+1998" in r["label"].replace("\n", " ")), have_matrix[0])
    ref_depths = {
        "Ice base":    (R_EUROPA - ref["R_ocean_med"])  / 1e3,
        "Seafloor":    (R_EUROPA - ref["R_mantle_med"]) / 1e3,
        "Core centre": R_EUROPA / 1e3,   # depth = R_EUROPA - r, so r=0 -> depth = R_EUROPA
    }

    print(f"\n{'─'*78}")
    print("  CREDIBLE-ENVELOPE OVERLAP ANALYSIS (16-84% bands, pairwise)")
    print(f"{'─'*78}")
    ...
    for i in range(len(labels)):
        for j in range(i + 1, len(labels)):
            ...
            lo_i, hi_i = bands[li]
            lo_j, hi_j = bands[lj]
            overlap_mask = (lo_i <= hi_j) & (lo_j <= hi_i)
            frac = 100 * overlap_mask.mean()
            print(f"\n  {li.strip():<28} vs {lj.strip():<28}"
                  f"  overlap over {frac:5.1f}% of the depth profile")
            for dname, dval in ref_depths.items():
                idx = int(np.argmin(np.abs(depth_grid - dval)))
                ov = lo_i[idx] <= hi_j[idx] and lo_j[idx] <= hi_i[idx]
                print(f"      at {dname:<12} (depth={dval:6.1f} km): "
                      f"[{lo_i[idx]:7.2f},{hi_i[idx]:7.2f}] vs "
                      f"[{lo_j[idx]:7.2f},{hi_j[idx]:7.2f}] °C  "
                      f"-> {'OVERLAP' if ov else 'DISTINCT (no overlap)'}")
    print()
...
if __name__ == "__main__":
    ...
    analyze_envelope_overlap(all_results)
